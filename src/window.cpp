...
void Window::draw(){
    ...
    DrawRectangleV(_position, Vector2{_size.x, 30}, moving? Color{50, 50, 50, 255} : BLACK);
    DrawRectangleLinesEx(Rectangle{_position.x, _position.y, _size.x, _size.y}, 2, BLACK);
    DrawTextEx(_font, _title.c_str(), Vector2{_position.x + 5, _position.y + 1}, 28, 0, WHITE);
}
